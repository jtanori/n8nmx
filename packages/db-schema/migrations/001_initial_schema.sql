...
-- 2. Tablas SIN dependencias primero
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_search_id') THEN
        ALTER TABLE leads ADD CONSTRAINT fk_search_id 
        FOREIGN KEY (search_id) REFERENCES search_queries(id) ON DELETE SET NULL;
    END IF;
END $$;
